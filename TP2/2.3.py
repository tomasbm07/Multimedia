...
def mfcc(pow_spec, sr, n_fft, hop_length, mel_filter, n_mfcc=13, lifter=0):
    ...
    mel_spec = np.where(
        mel_spec == 0, np.finfo(float).eps, mel_spec
    )  # Numerical Stability
...
def features(y, sr=22050, hop_length=512, n_mfcc=13, n_fft=2048):
    ...
    spec_centroid = np.apply_along_axis(spectral_centroid, 0, mag, f).reshape(
        1, mag.shape[1]
    )
    ...
    spec_flat = np.apply_along_axis(spectral_flateness, 0, power).reshape(
        1, mag.shape[1]
    )

    spec_rollof = np.apply_along_axis(spectral_rollof, 0, mag, f, 0.85).reshape(
        1, mag.shape[1]
    )
    ...
    return (mfccs, spec_centroid, spec_bdwidth, spec_flat, spec_rollof, rms, zcr)
